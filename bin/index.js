...
const breakLines = (content, width) => content
    .split('\n')
    .map((line) => wrapAnsi$1(line, width, { trim: false, hard: true }).split('\n'))
    .flat()
...
class ScreenManager {
    ...
    render(content, bottomContent = '') {
        ...
        const width = cliWidth$1({ defaultWidth: 80, output: this.rl.output });
        ...
        const backward = stringWidth$2(lastLine(output));
        if (backward > 0)
...
class Paginator {
    ...
    paginate(output, active, pageSize = 7) {
        ...
        const width = cliWidth$1({ defaultWidth: 80, output: process.stdout });
 ...
 */
class Separator {
    separator = chalk.dim(new Array(15).join(figures$1.line));
    type = 'separator';
...
function isSelectableChoice$1(choice) {
...
var checkbox = createPrompt((config, done) => {
    ...
    useKeypress((key) => {
        ...
        if (isEnterKey(key)) {
            setStatus('done');
                ...
                .filter((choice) => isSelectableChoice$1(choice) && choice.checked)
                .map((choice) => choice.value));
        }
        else if (isUpKey(key) || isDownKey(key)) {
            const offset = isUpKey(key) ? -1 : 1;
            let selectedOption;
            while (!isSelectableChoice$1(selectedOption)) {
                newCursorPosition =
            ...
            setChoices(choices.map((choice, i) => {
                if (i === cursorPosition && isSelectableChoice$1(choice)) {
            ...
            const selectAll = Boolean(choices.find((choice) => isSelectableChoice$1(choice) && !choice.checked));
            setChoices(choices.map((choice) => isSelectableChoice$1(choice) ? { ...choice, checked: selectAll } : choice));
        }
        else if (key.name === 'i') {
            setChoices(choices.map((choice) => isSelectableChoice$1(choice) ? { ...choice, checked: !choice.checked } : choice));
        }
        else if (isNumberKey(key)) {
            // Adjust index to start at 1
            const position = Number(key.name) - 1;
            // Abort if the choice doesn't exists or if disabled
            if (!isSelectableChoice$1(choices[position])) {
                return;
            }
            setCursorPosition(position);
            setChoices(choices.map((choice, i) => {
                if (i === position && isSelectableChoice$1(choice)) {
                    return { ...choice, checked: !choice.checked };
                }
                return choice;
            }));
    ...
    const allChoices = choices
        ...
        const checkbox = choice.checked
            ? chalk.green(figures$1.circleFilled)
            : figures$1.circle;
        if (index === cursorPosition) {
            return chalk.cyan(`${figures$1.pointer}${checkbox} ${line}`);
        }
...
var select = createPrompt((config, done) => {
    ...
    const allChoices = choices
        ...
        if (index === cursorPosition) {
            return chalk.cyan(`${figures$1.pointer} ${line}`);
        }
        return `  ${line}`;
    })
        .join('\n');
